...
(function() {
	...
	// // Buttons: buttons06.
	// 	$('#buttons06 > li:nth-child(1) > a').addEventListener(
	// 		'click',
	// 		function(event) { 
	// 			event.preventDefault();
	// 			event.stopPropagation();
	...
	// // Buttons: buttons15.
	// 	$('#buttons15 > li:nth-child(4) > a').addEventListener(
	// 		'click',
	// 		function(event) { 
	// 			event.preventDefault();
	// 			event.stopPropagation();
...
function getZuluTime() {
    fetch('../../backend/dispatchGetZuluTime.php').then(function (resp) {
        resp.text().then(function (text) {
			$('#zuluTime').html(text);
			setTimeout(getZuluTime, 900);
        });
    });
}

function getUnitsTable(server) {
    fetch(`../../backend/dispatchGetUnitTable.php?s=${server}`).then(function (resp) {
        resp.text().then(function (text) {
			$('#getUnitTable').html(text);
			setTimeout(getUnitsTable(server), 3000);
        });
    });
}

function getBoloTable(charId) {
    fetch(`../../backend/dispatchBoloTable.php?rId${charId}`).then(function (resp) {
        resp.text().then(function (text) {
			$('#boloAPI').html(text);
			setTimeout(getBoloTable(charId), 5000);
        });
    });
}

function getCallsTable(server, url) {
    fetch(`../../backend/dispatchGetCallsTable.php?s=${server}&url=${url}`).then(function (resp) {
        resp.text().then(function (text) {
			$('#callAPI').html(text);
			setTimeout(getCallsTable(server, url), 2000);
        });
    });
}

function getPanicStuff(server) {
    fetch(`../../backend/dispatchGetPanic.php?s=${server}`).then(function (resp) {
        resp.text().then(function (text) {
			$('#panicAPI').html(text);
			setTimeout(getPanicStuff(server), 7000);
        });
    });
}